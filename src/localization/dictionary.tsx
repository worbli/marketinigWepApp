interface TranslationsInterface {
  language: string,
  siteName: string,
  notFound: string,
  about: string,
  directionAim: string,
  keyDifferences: string,
  governanceComplience: string,
  getInvolved: string,
  launchOnWorbli: string,
  partnerWithUs: string,
  openAnAccount: string,
  progress: string,
  latestUpdates: string,
  applicationLaunches: string,
  technicalUpdates: string,
  whoWeAre: string,
  theTeam: string,
  foundation: string,
  ourPartners: string,
  pressReleases: string,
  contactUs: string,
  quickLinks: string,
  blockExplorer: string
  exchange: string
  reportAVulnrability: string
  blockProducers: string
  followUs: string
  medium: string
  twitter: string
  facebook: string
  youTube: string
  legal: string
  termsOfService: string
  privacy: string
  accessibility: string
  jurisdiction: string
  contact: string
  email: string
  support: string
  careers: string
  arbitration: string
  directionTitle: string
  directionSubTitle: string
  differencesTitle: string
  differencesSubTitle: string
  governanceTitle: string
  governanceSubTitle: string
  launchTitle: string
  launchSubTitle: string
  partnerTitle: string
  partnerSubTitle: string
  updatesTitle: string
  updatesSubTitle: string
  launchesTitle: string
  launchesSubTitle: string
  technicalTitle: string
  technicalSubTitle: string
  teamTitle: string
  teamSubTitle: string
  foundationTitle: string
  foundationSubTitle: string
  partnersTitle: string
  partnersSubTitle: string
  pressTitle: string
  pressSubTitle: string
  reportTitle: string
  reportSubTitle: string
  blockProducersTitle: string
  blockProducersSubTitle: string
  termsTitle: string
  termsSubTitle: string
  privacyTitle: string
  privacySubTitle: string
}

const Translations: { [key: string]: TranslationsInterface; } = {
  cn: {
    language: '中文',
    siteName: 'Worbli',
    notFound: 'Page not found!',
    about: 'About',
    directionAim: 'Direction / aim',
    keyDifferences: 'Key differences',
    governanceComplience: 'Governance & compliance',
    getInvolved: 'Get involved',
    launchOnWorbli: 'Launch on Worbli',
    partnerWithUs: 'Partner with us',
    openAnAccount: 'Open an account',
    progress: 'Progress',
    latestUpdates: 'Latest updates',
    applicationLaunches: 'Application launches',
    technicalUpdates: 'Technical updates',
    whoWeAre: 'Who we are',
    theTeam: 'The team',
    foundation: 'Foundation',
    ourPartners: 'Our partners',
    pressReleases: 'Press releases',
    contactUs: 'Contact us',
    quickLinks: 'Quick links',
    blockExplorer: 'Block Explorer',
    exchange: 'Exchange',
    reportAVulnrability: 'Report a vulnerability',
    blockProducers: 'Block Producers',
    followUs: 'Follow us',
    medium: 'Medium',
    twitter: 'Twitter',
    facebook: 'Facebook',
    youTube: 'YouTube',
    legal: 'Legal',
    termsOfService: 'Terms of service',
    privacy: 'Privacy',
    accessibility: 'Accessibility',
    jurisdiction: 'Jurisdiction',
    contact: 'Contact',
    email: 'Email',
    support: 'Support',
    careers: 'Careers',
    arbitration: 'Arbitration',
    directionTitle: 'string',
    directionSubTitle: 'string',
    differencesTitle: 'string',
    differencesSubTitle: 'string',
    governanceTitle: 'string',
    governanceSubTitle: 'string',
    launchTitle: 'string',
    launchSubTitle: 'string',
    partnerTitle: 'string',
    partnerSubTitle: 'string',
    updatesTitle: 'string',
    updatesSubTitle: 'string',
    launchesTitle: 'string',
    launchesSubTitle: 'string',
    technicalTitle: 'string',
    technicalSubTitle: 'string',
    teamTitle: 'string',
    teamSubTitle: 'string',
    foundationTitle: 'string',
    foundationSubTitle: 'string',
    partnersTitle: 'string',
    partnersSubTitle: 'string',
    pressTitle: 'string',
    pressSubTitle: 'string',
    reportTitle: 'string',
    reportSubTitle: 'string',
    blockProducersTitle: 'string',
    blockProducersSubTitle: 'string',
    termsTitle: 'string',
    termsSubTitle: 'string',
    privacyTitle: 'string',
    privacySubTitle: 'string',
  },
  kr: {
    language: '한국어',
    siteName: 'Worbli',
    notFound: 'Page not found!',
    about: 'About',
    directionAim: 'Direction / aim',
    keyDifferences: 'Key differences',
    governanceComplience: 'Governance & compliance',
    getInvolved: 'Get involved',
    launchOnWorbli: 'Launch on Worbli',
    partnerWithUs: 'Partner with us',
    openAnAccount: 'Open an account',
    progress: 'Progress',
    latestUpdates: 'Latest updates',
    applicationLaunches: 'Application launches',
    technicalUpdates: 'Technical updates',
    whoWeAre: 'Who we are',
    theTeam: 'The team',
    foundation: 'Foundation',
    ourPartners: 'Our partners',
    pressReleases: 'Press releases',
    contactUs: 'Contact us',
    quickLinks: 'Quick links',
    blockExplorer: 'Block Explorer',
    exchange: 'Exchange',
    reportAVulnrability: 'Report a vulnrability',
    blockProducers: 'Block Producers',
    followUs: 'Follow us',
    medium: 'Medium',
    twitter: 'Twitter',
    facebook: 'Facebook',
    youTube: 'YouTube',
    legal: 'Legal',
    termsOfService: 'Terms of service',
    privacy: 'Privacy',
    accessibility: 'Accessibility',
    jurisdiction: 'Jurisdiction',
    contact: 'Contact',
    email: 'Email',
    support: 'Support',
    careers: 'Careers',
    arbitration: 'Arbitration',
    directionTitle: 'string',
    directionSubTitle: 'string',
    differencesTitle: 'string',
    differencesSubTitle: 'string',
    governanceTitle: 'string',
    governanceSubTitle: 'string',
    launchTitle: 'string',
    launchSubTitle: 'string',
    partnerTitle: 'string',
    partnerSubTitle: 'string',
    updatesTitle: 'string',
    updatesSubTitle: 'string',
    launchesTitle: 'string',
    launchesSubTitle: 'string',
    technicalTitle: 'string',
    technicalSubTitle: 'string',
    teamTitle: 'string',
    teamSubTitle: 'string',
    foundationTitle: 'string',
    foundationSubTitle: 'string',
    partnersTitle: 'string',
    partnersSubTitle: 'string',
    pressTitle: 'string',
    pressSubTitle: 'string',
    reportTitle: 'string',
    reportSubTitle: 'string',
    blockProducersTitle: 'string',
    blockProducersSubTitle: 'string',
    termsTitle: 'string',
    termsSubTitle: 'string',
    privacyTitle: 'string',
    privacySubTitle: 'string',
  },
  en: {
    language: 'English',
    siteName: 'Worbli',
    notFound: 'Page not found!',
    about: 'About',
    directionAim: 'Direction / aim',
    keyDifferences: 'Key differences',
    governanceComplience: 'Governance & compliance',
    getInvolved: 'Get involved',
    launchOnWorbli: 'Launch on Worbli',
    partnerWithUs: 'Partner with us',
    openAnAccount: 'Open an account',
    progress: 'Progress',
    latestUpdates: 'Latest updates',
    applicationLaunches: 'Application launches',
    technicalUpdates: 'Technical updates',
    whoWeAre: 'Who we are',
    theTeam: 'The team',
    foundation: 'Foundation',
    ourPartners: 'Our partners',
    pressReleases: 'Press releases',
    contactUs: 'Contact us',
    quickLinks: 'Quick links',
    blockExplorer: 'Block Explorer',
    exchange: 'Exchange',
    reportAVulnrability: 'Report a vulnrability',
    blockProducers: 'Block Producers',
    followUs: 'Follow us',
    medium: 'Medium',
    twitter: 'Twitter',
    facebook: 'Facebook',
    youTube: 'YouTube',
    legal: 'Legal',
    termsOfService: 'Terms of service',
    privacy: 'Privacy',
    accessibility: 'Accessibility',
    jurisdiction: 'Jurisdiction',
    contact: 'Contact',
    email: 'Email',
    support: 'Support',
    careers: 'Careers',
    arbitration: 'Arbitration',
    directionTitle: 'string',
    directionSubTitle: 'string',
    differencesTitle: 'string',
    differencesSubTitle: 'string',
    governanceTitle: 'string',
    governanceSubTitle: 'string',
    launchTitle: 'string',
    launchSubTitle: 'string',
    partnerTitle: 'string',
    partnerSubTitle: 'string',
    updatesTitle: 'string',
    updatesSubTitle: 'string',
    launchesTitle: 'string',
    launchesSubTitle: 'string',
    technicalTitle: 'string',
    technicalSubTitle: 'string',
    teamTitle: 'string',
    teamSubTitle: 'string',
    foundationTitle: 'string',
    foundationSubTitle: 'string',
    partnersTitle: 'string',
    partnersSubTitle: 'string',
    pressTitle: 'string',
    pressSubTitle: 'string',
    reportTitle: 'string',
    reportSubTitle: 'string',
    blockProducersTitle: 'string',
    blockProducersSubTitle: 'string',
    termsTitle: 'string',
    termsSubTitle: 'string',
    privacyTitle: 'string',
    privacySubTitle: 'string',
  },
}
export { Translations }
